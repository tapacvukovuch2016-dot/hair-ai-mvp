...
st.title("✂️ Hair AI")
st.caption("MVP v0.32 — Natural Hairline")
        ...
        with open(image_path, "rb") as image_file, open(mask_path, "rb") as mask_file:
            r = client.images.edit(
                ...
                prompt=(
                    ...
                    "HAIRLINE FIX: preserve the source person's original frontal hairline position, contour and natural asymmetry. "
                    "Do not invent a new corner, notch, step, rectangular temple, sharp 90-degree angle, lineup, edge-up, or geometric forehead border. "
                    "In particular, the left and right frontal-temple transitions must follow the source hairline smoothly and naturally, "
                    "with tiny irregular individual hairs and gradual density changes rather than a stamped solid edge. "
                    "A Crew Cut must look barber-cut but NOT artificially lined up unless such a lineup already exists in the source photo. "
                    "NATURAL HAIRLINE: keep the macro shape and position of the source hairline, but render its boundary as real hair rather than a solid drawn border. "
                    "Use subtle sub-millimeter-looking irregularity, a few fine baby hairs, broken density and tiny gaps at the frontal edge; never make the whole front edge equally dark, equally sharp or perfectly continuous. "
                    "At both temples, taper density progressively into skin with soft individual hairs; do not create symmetric corners, boxed edges or a ruler-straight front line. "
                    "Keep these irregularities subtle and photographic: no fuzzy halo, no excessive flyaways, no lowered or raised hairline. "
                    ...
                    "Do not leave arcs, wisps, bands, duplicate hair, ghost hair, or the old hairstyle silhouette above or beside the new haircut. "
            ...
            if st.button("✨ Показать на мне",key="try_"+rec["id"],use_container_width=True):
                try:
                    ...
                    if out:
                        ...
                        st.caption("v0.32 Natural Hairline: сохраняет чистый силуэт v0.31, но делает переднюю линию и виски менее геометричными — с естественной микротекстурой и плавной плотностью.")
...
st.caption("v0.32 — Natural Hairline: чистый силуэт v0.31 + естественная линия роста волос и виски без нарисованного ровного края.")
