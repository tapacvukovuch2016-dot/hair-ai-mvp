...
st.title("✂️ Hair AI")
st.caption("MVP v0.13 — Style-Aware Hair Mask")
...
def _style_mask_profile(style_name: str):
    """
    v0.13: style-aware mask expansion.
    Different haircut families need different editable margins.
    """
    name = style_name.lower()

    # Defaults: balanced change around the existing hairstyle.
    profile = {
        "expand_x": 24,
        "expand_top": 28,
        "expand_bottom": 18,
    ...
    if any(k in name for k in short_keywords):
        # Short cuts need more freedom around temples/sides and hairline.
        profile.update({
            "expand_x": 52,
            "expand_top": 34,
            "expand_bottom": 34,
            "protect_face_top": 0.37,
            "protect_face_bottom": 0.92,
        })
    elif any(k in name for k in long_keywords):
        # Longer styles need room outside the current silhouette.
        profile.update({
            "expand_x": 72,
            "expand_top": 58,
            "expand_bottom": 70,
...
def _expand_polygon(polygon, size, profile):
    """
    Expand hairstyle polygon around its centroid, with extra vertical freedom.
    Keeps points inside the local crop.
    """
    w, h = size
    ...
    for x, y in polygon:
        ...
        nx = x + (profile["expand_x"] if dx >= 0 else -profile["expand_x"])

        if dy < 0:
            ny = y - profile["expand_top"]
        else:
            ny = y + profile["expand_bottom"]

        out.append((
            max(0, min(w - 1, int(nx))),
            max(0, min(h - 1, int(ny)))
        ))
    return out


def _polygon_masks(size, polygon, style_name):
    """
    v0.13:
    - expand the semantic hair polygon according to hairstyle family;
    - use the expanded polygon for generation freedom;
    - use a slightly tighter polygon for final compositing;
    - preserve the face with original pixels.
    """
    w, h = size
    profile = _style_mask_profile(style_name)

    expanded = _expand_polygon(polygon, size, profile)

    # API edit mask: opaque = protected, transparent = editable.
    edit = Image.new("RGBA", size, (255,255,255,255))
    e_draw = ImageDraw.Draw(edit)
    e_draw.polygon(expanded, fill=(255,255,255,0))

    # Final composite: take AI pixels in the expanded hairstyle region.
    alpha = Image.new("L", size, 0)
    a_draw = ImageDraw.Draw(alpha)
    a_draw.polygon(expanded, fill=255)

    # Strong face protection.
    face_box = (
        int(w * 0.245), int(h * profile["protect_face_top"]),
        int(w * 0.755), int(h * profile["protect_face_bottom"])
    )
    e_draw.ellipse(face_box, fill=(255,255,255,255))
    a_draw.ellipse(face_box, fill=0)

    # Feather boundaries for natural blending.
    edit_alpha = edit.getchannel("A").filter(ImageFilter.GaussianBlur(radius=6))
    edit.putalpha(edit_alpha)
    alpha = alpha.filter(ImageFilter.GaussianBlur(radius=5))

    return edit, alpha, expanded

def generate_tryon(style_name):
    """
    v0.12 Semantic Hair Polygon Composite
    ...
    try:
        ...
        edit_mask, composite_alpha, expanded_polygon = _polygon_masks(work_size, polygon, style_name)
        ...
        with open(image_path, "rb") as image_file, open(mask_path, "rb") as mask_file:
            r = client.images.edit(
                ...
                prompt=(
                    ...
                    "Do not alter the person's face, expression, glasses, skin, teeth, ears, clothing, lighting or any background. "
                    "The result must look like the same original photo after a professional salon haircut."
                ),
                size="1024x1024",
                quality="high"
            )
        ...
        # Critical v0.12 step:
        # AI contributes ONLY semantic hair-polygon pixels.
            ...
            if st.button("✨ Показать на мне",key="try_"+rec["id"],use_container_width=True):
                try:
                    ...
                    if out:
                        ...
                        st.caption("AI-визуализация — ориентир. v0.13 расширяет область волос по-разному для коротких, объёмных и длинных стрижек, сохраняя лицо и фон из оригинала.")
...
st.caption("v0.13 — экспериментальный прототип. Не оценивает красоту человека.")
