...
st.title("✂️ Hair AI")
st.caption("MVP v0.15 — Locked Hair + Timing")
...
def _semantic_hair_alpha(size, polygon, style_name):
    """
    Build a style-aware editable mask from the actual detected hairstyle.

    Returns:
      edit_mask RGBA:
        transparent only near detected hair; opaque elsewhere.
      composite_alpha L:
        AI pixels accepted only near detected hair.
    """
    w, h = size
    profile = _style_mask_profile(style_name)

    # Base semantic hairstyle mask.
    base = Image.new("L", size, 0)
    draw = ImageDraw.Draw(base)
    draw.polygon(polygon, fill=255)

    # Controlled dilation. Pillow MaxFilter requires odd kernel size.
    radius = max(1, int(profile["dilate_px"]))
    kernel = radius * 2 + 1
    if kernel > 99:
        kernel = 99
    if kernel % 2 == 0:
        kernel += 1
    expanded = base.filter(ImageFilter.MaxFilter(kernel))

    # Give volume styles a little extra room directly above the detected hair,
    # without expanding sideways into unrelated background.
    top_extra = int(profile["extra_top_px"])
    if top_extra > 0:
        bbox = base.getbbox()
        if bbox:
            l, t, r, b = bbox
            ...
            ed.rectangle(
                (
                    ...
                    max(0, t - top_extra),
                    min(w - 1, r),
                    min(h - 1, t + max(6, top_extra // 3))
                ),
                fill=255
            )
            from PIL import ImageChops
            expanded = ImageChops.lighter(expanded, extra)

    # v0.15: strict spatial clamp. Even if dilation grows, AI may only affect
    # a bounded rectangle around the actually detected hair. Everything outside
    # this box is forced back to the original image during compositing.
    bbox = base.getbbox()
    if bbox:
        l, t, r, b = bbox
        pad_x = max(12, int((r - l) * 0.10))
        pad_top = max(10, int(profile["extra_top_px"]))
        pad_bottom = max(8, int((b - t) * 0.06))
        ...
        from PIL import ImageChops
        expanded = ImageChops.darker(expanded, clamp)

    # Hard protection for facial area.
    ...
    fd.ellipse(face_box, fill=255)

    # Remove face from editable/composite region.
    from PIL import ImageChops
    expanded = ImageChops.subtract(expanded, face_protect)

    # Very slight feather at the boundary.
    composite_alpha = expanded.filter(ImageFilter.GaussianBlur(radius=1.5))

    # API mask: opaque protected, transparent editable.
    edit_alpha = Image.eval(expanded, lambda p: 255 - p)
    edit_alpha = edit_alpha.filter(ImageFilter.GaussianBlur(radius=1.5))
            ...
            if st.button("✨ Показать на мне",key="try_"+rec["id"],use_container_width=True):
                try:
                    ...
                    if out:
                        ...
                        st.caption("AI-визуализация — ориентир. v0.15 сильнее защищает фон и показывает время генерации.")
...
st.caption("v0.15 — экспериментальный прототип. Не оценивает красоту человека.")
