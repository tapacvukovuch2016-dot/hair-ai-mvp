...
st.title("✂️ Hair AI")
st.caption("MVP v0.11 — Full Hair Silhouette Edit")
...
def _local_head_crop_box(source: Image.Image):
    ...
    # v0.11: larger local crop around the whole head silhouette.
    # Still local, but gives the model enough room to reshape a haircut.
...
def _build_local_edit_mask(size=(1024, 1024)) -> Image.Image:
    ...
    # Editable full hairstyle silhouette: top, crown, temples, sides and upper ears region.
    draw.ellipse(
        (int(w*0.06), int(h*0.00), int(w*0.94), int(h*0.76)),
        fill=(255, 255, 255, 0)
    )

    # Strongly protect the core facial area, but leave the hairline and side hair editable.
    draw.ellipse(
        (int(w*0.255), int(h*0.36), int(w*0.745), int(h*0.91)),
...
def _build_local_composite_alpha(size=(1024, 1024)) -> Image.Image:
    ...
    # v0.11 accepts AI pixels over the full hairstyle silhouette so the haircut
    # can change visibly, including crown, top, temples and side hair.
    draw.ellipse(
        (int(w*0.07), int(h*0.00), int(w*0.93), int(h*0.75)),
        fill=255
    )

    # Physically restore the main facial area from the original crop.
    draw.ellipse(
        (int(w*0.26), int(h*0.355), int(w*0.74), int(h*0.915)),
        fill=0
    )
    ...
    try:
        ...
        with open(image_path, "rb") as image_file, open(mask_path, "rb") as mask_file:
            r = client.images.edit(
                ...
                mask=mask_file,
                prompt=(
                    f"Transform the current hairstyle into a clearly recognizable, salon-realistic {style_name}. "
                    "Make the haircut structurally faithful to that style, not a subtle variation of the current hair. "
                    "You may substantially change hair length, top volume, crown shape, temples, side hair and the outer silhouette "
                    "of the hairstyle when needed for the requested cut. "
                    "For short styles such as Crew Cut, visibly shorten the top and sides so the result unmistakably reads as that haircut. "
                    "Keep the exact same person, head pose and facial expression. Preserve natural hair color, believable density, "
                    "visible scalp and a realistic hairline. Do not change eyes, eyebrows, nose, mouth, teeth, smile, skin, glasses, "
                    "facial hair, neck, clothing, lighting, perspective or background. "
                    "The final image should look like the same original photograph after a professional haircut."
                ),
                size="1024x1024",
                quality="high"
            )
            ...
            if st.button("✨ Показать на мне",key="try_"+rec["id"],use_container_width=True):
                try:
                    ...
                    if out:
                        ...
                        st.caption("AI-визуализация — ориентир. v0.11 даёт AI больше свободы над всей формой причёски, но лицо и фон остаются защищёнными.")
...
st.caption("v0.11 — экспериментальный прототип. Не оценивает красоту человека.")
