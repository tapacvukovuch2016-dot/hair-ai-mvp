    ...
    try:
        ...
        with open(p, "rb") as image_file:
            r = client.images.edit(
                ...
                image=image_file,
                prompt=(
                    f"Perform a localized hairstyle edit: change ONLY the hair region to a realistic {style_name}. "
                    "Treat every pixel outside the hair and immediately adjacent hair boundary as protected reference content. "
                    "Do not redraw or reinterpret the face. Preserve identity and facial geometry exactly, including eyes, "
                    "eyebrows, nose, cheeks, jaw, mouth, lips, teeth, smile, ears, skin texture, glasses and facial hair. "
                    "Also preserve head pose, neck, body, clothing, hands, objects, background, camera perspective, crop and lighting. "
                    "Preserve the person's natural hair color, plausible density, visible hairline and scalp characteristics. "
                    "Only modify pixels necessary to create the requested haircut and blend its boundary naturally into the original photo. "
                    "No face retouching, beautification, skin smoothing, relighting or background regeneration. "
                    "The output should look like the same original photograph after a real salon haircut."
                ),
                size="1024x1024",
                quality="medium"
            )
            ...
            if st.button("✨ Показать на мне",key="try_"+rec["id"],use_container_width=True):
                try:
                    ...
                    if out:
                        ...
                        st.caption("AI-визуализация — ориентир. v0.6 старается менять только волосы; мелкие детали исходного фото всё ещё могут отличаться.")
