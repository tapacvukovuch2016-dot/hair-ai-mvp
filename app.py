...
st.title("✂️ Hair AI")
st.caption("MVP v0.25 — Precise Hair Mask")
            ...
            if st.button("✨ Показать на мне",key="try_"+rec["id"],use_container_width=True):
                try:
                    ...
                    if out:
                        ...
                        st.caption("v0.25 Precise Mask: более компактная зона редактирования вокруг волос, чтобы не перерисовывать фон над головой.")
...
st.caption("v0.25 — Precise Mask: сохраняет кэш и identity lock, но сужает/смягчает маску для устранения ореола и перерисовки фона.")
