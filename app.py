...
st.title("✂️ Hair AI")
st.caption("MVP v0.30 — Hairline Fix")
...
def _semantic_hair_alpha(size, polygon, style_name):
    ...
    ring = ring.point(lambda p: int(p * 0.25))
    ...
    hard_guard = dilate(base, api_radius + 2).filter(ImageFilter.GaussianBlur(radius=0.9))
            ...
            if st.button("✨ Показать на мне",key="try_"+rec["id"],use_container_width=True):
                try:
                    ...
                    if out:
                        ...
                        st.caption("v0.30 Hairline Fix: сохраняет натуральную текстуру v0.29 и дополнительно запрещает искусственные углы, ступеньки и новую линию роста волос.")
...
st.caption("v0.30 — Hairline Fix: натуральная текстура v0.29 + сохранение исходной линии роста волос без ступенек, острых углов и искусственного lineup.")
