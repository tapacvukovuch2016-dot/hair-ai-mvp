...
st.title("✂️ Hair AI")
st.caption("MVP v0.18 — Faster Try-On + Pixel-Locked Background")
...
def _semantic_hair_alpha(size, polygon, style_name):
    ...
    if bbox:
        ...
        pad_x = max(24, int((r - l) * 0.13))
        pad_bottom = max(16, int((b - t) * 0.07))
        cd.rectangle(
            (
                max(0, l - pad_x),
                max(0, t - top_room - 8),
    ...
    # Final composite: full AI in inner zone, progressively less AI in the ring.
    # This is the key v0.16 change: background pixels outside the ring are
    # guaranteed to come from the original.
    inner_soft = inner.filter(ImageFilter.GaussianBlur(radius=2.5))
    outer_soft = outer.filter(ImageFilter.GaussianBlur(radius=6.0))

    # Blend weights: inner dominates; transition ring is intentionally weaker.
    composite_alpha = ImageChops.lighter(
        inner_soft,
        outer_soft.point(lambda p: int(p * 0.22))
    )
...
def generate_tryon(style_name, quality="medium"):
    """
    v0.18 Faster Dual-Zone Hair Mask
    ...
    cached_tryon = st.session_state.get("tryon_cache", {}).get(cache_key)
    if cached_tryon:
        cached_bytes, cached_timings = cached_tryon
...
if r:
    ...
    if r.get("recommendations"):
        ...
        for rec in r["recommendations"]:
            ...
            with qcol2:
                st.caption("HD качественнее, но медленнее")
            if st.button("✨ Показать на мне",key="try_"+rec["id"],use_container_width=True):
                try:
                    api_quality = "low" if quality_choice == "Быстро" else "medium"
                    with st.status("Создаю визуальную примерку...", expanded=True) as status:
                        st.write("Подготавливаю фото и маску волос…")
                        result=generate_tryon(rec["name"], api_quality)
                        out, timings = result
                        status.update(label="Готово", state="complete", expanded=False)
                    if out:
                        st.image(out,caption=f"Визуализация: {rec['name']}",use_container_width=True)
                        if timings.get("result_cached"):
                            st.success("⚡ Эта примерка взята из кэша — повторная генерация не потребовалась.")
                        cached = "из кэша" if timings.get("hair_analysis_cached") else f"{timings.get('hair_analysis',0):.1f} с"
                        st.caption(f"⏱ Анализ волос: {cached} · Генерация: {timings.get('generation',0):.1f} с · Обработка: {timings.get('processing',0):.1f} с · Всего: {timings.get('total',0):.1f} с")
                        st.caption("AI-визуализация — ориентир. v0.18: Fast=low, HD=medium; повторная примерка того же варианта берётся из кэша.")
...
st.caption("v0.18 — экспериментальный прототип. Fast=low, HD=medium, кэш готовых примерок и более жёсткая фиксация фона.")
