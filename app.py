...
st.title("✂️ Hair AI")
st.caption("MVP v0.35 — Women's Styles Test")
...
def _semantic_hair_alpha(size, polygon, style_name):
    ...
    name = style_name.lower()
    if any(k in name for k in ["crew cut", "buzz", "caesar", "crop", "fade", "taper", "ivy league", "pixie"]):
        api_radius = 29
        core_radius = 18
        feather_radius = 8
            ...
            if st.button("✨ Показать на мне",key="try_"+rec["id"],use_container_width=True):
                try:
                    ...
                    if out:
                        ...
                        st.caption("v0.35: сохраняет Hairstyle Identity и добавляет отдельные геометрические инструкции для женских причёсок.")
...
st.caption("v0.35 — Women's Styles Test: добавлен отдельный тестовый каталог женских причёсок без ограничения по полу пользователя.")
