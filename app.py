...
st.title("✂️ Hair AI")
st.caption("MVP v0.12 — Semantic Hair-Only Composite")
...
def _polygon_masks(size, polygon):
    """
    Returns:
      edit_mask RGBA: transparent inside editable hair polygon.
      composite_alpha L: white only inside hair polygon.

    A protected face ellipse is removed from both masks so the final face is
    physically copied from the original crop.
    """
    w, h = size
    ...
    e_draw.polygon(polygon, fill=(255,255,255,0))

    # Final composite: generated pixels only inside semantic hairstyle polygon.
    alpha = Image.new("L", size, 0)
    a_draw = ImageDraw.Draw(alpha)
    a_draw.polygon(polygon, fill=255)

    # Strong face protection.
    face_box = (
        int(w * 0.255), int(h * 0.34),
        int(w * 0.745), int(h * 0.91)
    )
    e_draw.ellipse(face_box, fill=(255,255,255,255))
    a_draw.ellipse(face_box, fill=0)

    # Light feather just at the hairstyle boundary.
    edit_alpha = edit.getchannel("A").filter(ImageFilter.GaussianBlur(radius=5))
...
def generate_tryon(style_name):
    ...
    try:
        ...
        edit_mask, composite_alpha = _polygon_masks(work_size, polygon)
        ...
        with open(image_path, "rb") as image_file, open(mask_path, "rb") as mask_file:
            r = client.images.edit(
                ...
                mask=mask_file,
                prompt=(
                    f"Transform only the editable hairstyle area into a clearly recognizable, "
                    f"photorealistic {style_name}. Make it structurally faithful to that haircut, "
                    "not a subtle variation. You may change hair length, top volume, crown, temples, "
                    "side hair and hairstyle silhouette inside the editable region. "
                    "For a Crew Cut, visibly shorten the top and sides. "
                    "Preserve natural hair color, believable density, visible scalp and realistic hairline. "
                    "Do not alter the person's face, expression, glasses, skin, teeth, ears, clothing, "
                    "lighting or any background. The result must look like the same original photo after a salon haircut."
                ),
                size="1024x1024",
                quality="high"
            )
            ...
            if st.button("✨ Показать на мне",key="try_"+rec["id"],use_container_width=True):
                try:
                    ...
                    if out:
                        ...
                        st.caption("AI-визуализация — ориентир. v0.12 автоматически определяет область волос и принимает от AI только пиксели этой зоны; фон и лицо берутся из оригинала.")
...
st.caption("v0.12 — экспериментальный прототип. Не оценивает красоту человека.")
