...
st.title("✂️ Hair AI")
st.caption("MVP v0.26 — Hair-Only Mask")
...
def _semantic_hair_alpha(size, polygon, style_name):
    """
    v0.26 Hair-Only Mask:
    accept AI pixels only inside detected hair and a narrow feather edge.
    """
    ...
    if any(k in name for k in ["crew cut", "buzz", "caesar", "crop", "fade", "taper", "ivy league"]):
        inner_radius, api_radius, edge_radius = 9, 16, 5
    elif any(k in name for k in ["bro flow", "shoulder", "long center", "wolf", "shag", "curtain", "mullet", "undercut"]):
        inner_radius, api_radius, edge_radius = 14, 24, 7
    else:
        inner_radius, api_radius, edge_radius = 11, 20, 6
    ...
    # Protect the facial core while leaving the natural hairline available.
    ...
    # AI edit zone follows the real hair polygon, not the whole head.
    api_zone = dilate(base, api_radius)
    api_zone = ImageChops.subtract(api_zone, face_protect)
    api_zone = api_zone.filter(ImageFilter.GaussianBlur(radius=1.5))

    edit_alpha = Image.eval(api_zone, lambda p: 255 - p)
    edit_mask = Image.new("RGBA", size, (255, 255, 255, 255))
    edit_mask.putalpha(edit_alpha)

    # Final accepted AI pixels are even tighter than the edit zone.
    inner = dilate(base, inner_radius)
    inner = ImageChops.subtract(inner, face_protect)
    inner_soft = inner.filter(ImageFilter.GaussianBlur(radius=0.9))

    edge_outer = dilate(inner, edge_radius).filter(ImageFilter.GaussianBlur(radius=1.8))
    edge_ring = ImageChops.subtract(edge_outer, inner_soft)
    edge_ring = edge_ring.point(lambda p: int(p * 0.12))

    composite_alpha = ImageChops.lighter(inner_soft, edge_ring)

    # Hard guard: no generated pixel can survive far from actual hair.
    guard = dilate(base, api_radius + 2).filter(ImageFilter.GaussianBlur(radius=0.8))
    composite_alpha = ImageChops.darker(composite_alpha, guard)
            ...
            if st.button("✨ Показать на мне",key="try_"+rec["id"],use_container_width=True):
                try:
                    ...
                    if out:
                        ...
                        st.caption("v0.26 Hair-Only Mask: AI принимается только внутри волос и очень узкой кромки по их контуру.")
...
st.caption("v0.26 — Hair-Only Mask: фон восстанавливается из оригинала, а редактирование ограничено волосами и узкой кромкой.")
