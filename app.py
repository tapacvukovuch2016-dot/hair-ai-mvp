...
st.title("✂️ Hair AI")
st.caption("MVP v0.27 — Natural Blend")
...
def _semantic_hair_alpha(size, polygon, style_name):
    ...
    core = core.filter(ImageFilter.GaussianBlur(radius=1.15))
    ...
    outer = outer.filter(ImageFilter.GaussianBlur(radius=3.2))

    ring = ImageChops.subtract(outer, core)
    ring = ring.point(lambda p: int(p * 0.28))
            ...
            if st.button("✨ Показать на мне",key="try_"+rec["id"],use_container_width=True):
                try:
                    ...
                    if out:
                        ...
                        st.caption("v0.27 Natural Blend: Hair-Only Mask + мягкое смешивание по границе волос без возврата ореола вокруг головы.")
...
st.caption("v0.27 — Natural Blend: сохраняет Hair-Only Mask и добавляет естественный переход между исходными и сгенерированными волосами.")
