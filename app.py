...
st.title("✂️ Hair AI")
st.caption("MVP v0.14 — Pixel-Locked Hair Mask")
...
def _semantic_hair_alpha(size, polygon, style_name):
    ...
    composite_alpha = expanded.filter(ImageFilter.GaussianBlur(radius=3))

    # API mask: opaque protected, transparent editable.
    edit_alpha = Image.eval(expanded, lambda p: 255 - p)
    edit_alpha = edit_alpha.filter(ImageFilter.GaussianBlur(radius=3))
...
def generate_tryon(style_name):
    """
    v0.14 Pixel-Locked Semantic Hair Composite
            ...
            if st.button("✨ Показать на мне",key="try_"+rec["id"],use_container_width=True):
                try:
                    ...
                    if out:
                        ...
                        st.caption("AI-визуализация — ориентир. v0.14 держит область редактирования вплотную к реально найденным волосам, чтобы фон оставался пиксельно исходным.")
...
st.caption("v0.14 — экспериментальный прототип. Не оценивает красоту человека.")
