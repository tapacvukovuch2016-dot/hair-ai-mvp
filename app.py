...
st.title("✂️ Hair AI")
st.caption("MVP v0.20 — Smooth Hair Contour + Full Timing")
            ...
            if st.button("✨ Показать на мне",key="try_"+rec["id"],use_container_width=True):
                try:
                    ...
                    if out:
                        ...
                        st.caption(
                            f"⏱ Подготовка: {timings.get('prepare',0):.1f} с · "
...
st.caption("v0.20 — Smooth Hair Contour + Full Timing: убран прямоугольный clamp, добавлена плавная контурная маска и детальные тайминги этапов.")
