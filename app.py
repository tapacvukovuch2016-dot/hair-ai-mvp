...
st.title("✂️ Hair AI")
st.caption("MVP v0.10 — Stronger Local Hair Edit")
...
def _local_head_crop_box(source: Image.Image):
    ...
    # Wider crop for v0.10: include temples, side hair and more head silhouette.
...
def _build_local_edit_mask(size=(1024, 1024)) -> Image.Image:
    ...
    # Wider editable hair/head zone including temples and side hair.
    draw.ellipse(
        (int(w*0.10), int(h*0.01), int(w*0.90), int(h*0.70)),
        fill=(255, 255, 255, 0)
    )

    # Protect most of the face. Hairline/temples remain editable.
    draw.ellipse(
        (int(w*0.27), int(h*0.34), int(w*0.73), int(h*0.88)),
...
def _build_local_composite_alpha(size=(1024, 1024)) -> Image.Image:
    ...
    # v0.10 lets generated pixels affect more of the hairstyle silhouette:
    # top, temples and side hair.
    draw.ellipse(
        (int(w*0.11), int(h*0.02), int(w*0.89), int(h*0.69)),
        fill=255
    )

    # Physically restore the central/lower face from the original crop.
    draw.ellipse(
        (int(w*0.275), int(h*0.335), int(w*0.725), int(h*0.89)),
        fill=0
    )
    ...
    try:
        ...
        with open(image_path, "rb") as image_file, open(mask_path, "rb") as mask_file:
            r = client.images.edit(
                ...
                mask=mask_file,
                prompt=(
                    f"Change the hairstyle in this local head crop to a clearly recognizable, photorealistic {style_name}. "
                    "The haircut must be visibly different from the current hairstyle when the requested style calls for "
                    "shorter sides, shorter top, tapering, fade, texture or a changed silhouette. "
                    "You may modify the hair at the top, temples and side hair, but keep the exact same person and head pose. "
                    "Preserve natural hair color, believable density, visible scalp and a realistic hairline. "
                    "Do not change eyes, eyebrows, nose, mouth, teeth, smile, ears, skin, glasses, facial hair, "
                    "lighting, perspective or background visible in the crop. "
                    "The output should look like the same original photograph immediately after a professional salon haircut."
                ),
                size="1024x1024",
                quality="high"
            )
            ...
            if st.button("✨ Показать на мне",key="try_"+rec["id"],use_container_width=True):
                try:
                    ...
                    if out:
                        ...
                        st.caption("AI-визуализация — ориентир. v0.10 расширяет локальную область волос, чтобы сильнее менять верх и виски, сохраняя лицо и фон.")
...
st.caption("v0.10 — экспериментальный прототип. Не оценивает красоту человека.")
