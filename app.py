...
st.title("✂️ Hair AI")
st.caption("MVP v0.22 — Speed Mode")
...
def _semantic_hair_alpha(size, polygon, style_name):
    ...
    def dilate(mask, radius):
        """
        v0.22 fast morphology.
        MaxFilter is expensive on a 1024x1024 mask, so perform dilation at
        half resolution and scale it back. For a soft hairstyle mask this is
        visually equivalent enough, but much faster.
        """
        mw, mh = mask.size
        if mw >= 768 and mh >= 768:
            small = mask.resize((mw // 2, mh // 2), Image.Resampling.NEAREST)
            small_radius = max(1, int(round(radius / 2)))
            kernel = min(49, small_radius * 2 + 1)
            if kernel % 2 == 0:
                kernel -= 1
            ...
            if st.button("✨ Показать на мне",key="try_"+rec["id"],use_container_width=True):
                try:
                    ...
                    if out:
                        ...
                        st.caption("v0.22 Speed: ускоренная маска, быстрый lossless PNG, Fast=low, HD=medium и кэш готовых примерок.")
...
st.caption("v0.22 — Speed Mode: ускоряет маску и финальную обработку без изменения пикселей вне зоны волос; сохраняет детальные тайминги.")
