...
st.title("✂️ Hair AI")
st.caption("MVP v0.8 — Hair Composite Try-On")
...
def _build_hair_edit_mask(source: Image.Image) -> Image.Image:
    """
    v0.8 conservative editable mask around the upper head/hair.
    ...
    # Protect most of the face.
    face_left = int(w * 0.29)
    face_right = int(w * 0.71)
    face_top = int(h * 0.18)
    face_bottom = int(h * 0.52)
    draw.ellipse((face_left, face_top, face_right, face_bottom), fill=(255, 255, 255, 255))

    alpha = mask.getchannel("A").filter(
        ImageFilter.GaussianBlur(radius=max(2, int(w * 0.008)))
    )
    mask.putalpha(alpha)
    return mask


def _build_composite_alpha(source: Image.Image) -> Image.Image:
    """
    Alpha used AFTER generation.
    White = take generated pixels.
    Black = keep original pixels.

    This is intentionally narrower than the edit mask so face/body/background
    are physically copied from the original image in the final result.
    """
    w, h = source.size
    ...
    # Cut out the central/lower face from generated region.
    face_left = int(w * 0.30)
    face_right = int(w * 0.70)
    face_top = int(h * 0.20)
    face_bottom = int(h * 0.53)
    draw.ellipse((face_left, face_top, face_right, face_bottom), fill=0)

    return alpha.filter(ImageFilter.GaussianBlur(radius=max(2, int(w * 0.006))))


def _fit_generated_to_source(generated: Image.Image, source: Image.Image) -> Image.Image:
    """
    Center-crop generated result to source aspect ratio, then resize to the
    exact source dimensions so compositing is pixel-aligned.
    """
    generated = generated.convert("RGB")
    target_ratio = source.width / source.height
    gen_ratio = generated.width / generated.height

    if abs(gen_ratio - target_ratio) > 0.001:
    ...
    return generated.resize(source.size, Image.Resampling.LANCZOS)
...
def generate_tryon(style_name):
    ...
    try:
        ...
        # Preserve source dimensions for the final composite.
        work = source.copy()
        max_side = 1536
        if max(work.size) > max_side:
            work.thumbnail((max_side, max_side), Image.Resampling.LANCZOS)

        edit_mask = _build_hair_edit_mask(work)
        ...
        work.save(image_path, format="PNG")
        ...
        with open(image_path, "rb") as image_file, open(mask_path, "rb") as mask_file:
            r = client.images.edit(
                ...
                mask=mask_file,
                prompt=(
                    f"Create a photorealistic {style_name} in the editable upper-head region only. "
                    "Keep natural hair color, believable density, scalp visibility and hairline. "
                    "Do not alter face, glasses, teeth, skin, ears, body, clothing, objects, background or lighting. "
                    "Keep the head perspective consistent with the original photo. "
                    "This should look like the same person after a real salon haircut."
                ),
                size="1024x1024",
                quality="medium"
            )
        ...
        # Align generated image to the working source.
        generated_fit = _fit_generated_to_source(generated, work)

        # Physical post-generation composite:
        # only hairstyle-area pixels are allowed to come from AI.
        composite_alpha = _build_composite_alpha(work)
        composited = Image.composite(generated_fit, work, composite_alpha)

        # Return to original source dimensions.
        if composited.size != source.size:
            composited = composited.resize(source.size, Image.Resampling.LANCZOS)

        out = BytesIO()
        composited.save(out, format="JPEG", quality=96)
        return out.getvalue()
            ...
            if st.button("✨ Показать на мне",key="try_"+rec["id"],use_container_width=True):
                try:
                    ...
                    if out:
                        ...
                        st.caption("AI-визуализация — ориентир. v0.8 физически возвращает оригинальные пиксели лица, тела и фона после генерации.")
...
st.caption("v0.8 — экспериментальный прототип. Не оценивает красоту человека.")
