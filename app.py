...
st.title("✂️ Hair AI")
st.caption("MVP v0.19 — Tight Hair Contour Lock")
...
def _semantic_hair_alpha(size, polygon, style_name):
    ...
    narrow_outer = dilate(inner, 8)
    narrow_outer = narrow_outer.filter(ImageFilter.GaussianBlur(radius=2.2))
    ring = ImageChops.subtract(narrow_outer, inner_soft)
    ring = ring.point(lambda p: int(p * 0.10))

    composite_alpha = ImageChops.lighter(inner_soft, ring)

    # Hard spatial clamp around actual detected hair bbox.
    # This guarantees that far-away background pixels cannot come from AI.
    bbox2 = base.getbbox()
    if bbox2:
        l2, t2, r2, b2 = bbox2
        clamp2 = Image.new("L", size, 0)
        c2 = ImageDraw.Draw(clamp2)
        pad_x2 = max(10, int((r2 - l2) * 0.07))
        pad_top2 = max(8, int((b2 - t2) * 0.05))
        pad_bottom2 = max(8, int((b2 - t2) * 0.05))
        c2.rectangle(
            (
                max(0, l2 - pad_x2),
                max(0, t2 - pad_top2),
                min(w - 1, r2 + pad_x2),
                min(h - 1, b2 + pad_bottom2),
            ),
            fill=255,
        )
        composite_alpha = ImageChops.darker(composite_alpha, clamp2)
...
st.caption("v0.19 — Tight Hair Contour Lock: ещё более узкая переходная зона вокруг волос, чтобы фон сохранялся почти пиксель-в-пиксель.")
